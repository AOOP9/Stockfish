...
#define S(mg, eg) make_score(mg, eg)
  ...
  constexpr Score DropSafeBonus = S(30, 30);
#endif
#ifdef RACE
...
#undef S
  ...
  Score Evaluation<T>::variant() const {
    ...
    if (pos.is_house())
    {
        // Bonus for unoccupied squares in opponent area that are not attacked by opponent non-KQ pieces
        score += DropSafeBonus * popcount(~(attackedBy[BLACK][PAWN] | attackedBy[BLACK][KNIGHT] | attackedBy[BLACK][BISHOP] | attackedBy[BLACK][ROOK] | pos.pieces()) & (Rank5BB | Rank6BB | Rank7BB | Rank8BB));
        score -= DropSafeBonus * popcount(~(attackedBy[WHITE][PAWN] | attackedBy[WHITE][KNIGHT] | attackedBy[WHITE][BISHOP] | attackedBy[WHITE][ROOK] | pos.pieces()) & (Rank1BB | Rank2BB | Rank3BB | Rank4BB));
    }
#endif
#ifdef HORDE
